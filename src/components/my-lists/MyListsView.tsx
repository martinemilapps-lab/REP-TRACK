...
const VISIT_CYCLES = [7, 10, 14, 30] as const;
export function MyListsView({ reps, selectedRep, onSelectRep, onLogVisitForCustomer, readOnly = false, }: MyListsViewProps) {
    ...
    const handleSaveModal = async (e: React.FormEvent) => {
        ...
        if (!VISIT_CYCLES.includes(Number(modalFormData.defaultCycle) as typeof VISIT_CYCLES[number])) {
            showNotification(language === 'ar' ? 'اختر دورة زيارة من 7 أو 10 أو 14 أو 30 يوماً' : 'Choose a visit cycle of 7, 10, 14, or 30 days', true);
            return;
        }
        setSaving(true);
 ...
 <Drawer open={isModalOpen} title={editingItem ? l('Edit customer', 'تعديل العميل') : l('Add customer', 'إضافة عميل')} onClose={() => { if (!saving)
        setIsModalOpen(false); }}>
    <form onSubmit={handleSaveModal}>{statusMsg?.isError && <InlineAlert tone="error">{statusMsg.text}</InlineAlert>}<fieldset disabled={saving} className="grid min-w-0 gap-4 sm:grid-cols-2">{activeCategory==='hospitals'&&<fieldset className="sm:col-span-2"><legend className="text-sm font-semibold">{l('Hospital type','نوع المستشفى')} *</legend><div className="mt-2 flex flex-wrap gap-3">{['Private','Government','University','Insurance','Other'].map(type=><label key={type} className="flex items-center gap-2"><input type="checkbox" checked={(modalFormData.hospitalTypes||[]).includes(type)} onChange={e=>setModalFormData({...modalFormData,hospitalTypes:e.target.checked?[...(modalFormData.hospitalTypes||[]),type]:(modalFormData.hospitalTypes||[]).filter(x=>x!==type),type:e.target.checked?type:modalFormData.type})}/>{type}</label>)}</div></fieldset>}<label className="text-sm font-semibold">{fields.defaultCycle[ar ? 1 : 0]} *<select required className="input mt-1 w-full" value={String(modalFormData.defaultCycle ?? 7)} onChange={e=>setModalFormData({...modalFormData,defaultCycle:Number(e.target.value)})}>{VISIT_CYCLES.map(value=><option key={value} value={value}>{value}</option>)}</select></label>{keys[activeCategory].map(key => <FormField key={key} label={fields[key][ar ? 1 : 0]} value={String(modalFormData[key as keyof CustomerFields] ?? '')} required={key === 'name'} onChange={value => setModalFormData({
