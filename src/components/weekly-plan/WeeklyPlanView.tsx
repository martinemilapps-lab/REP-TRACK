...
import {Button} from '@/components/ui/Button';import {FormField} from '@/components/ui/FormField';import {FormSection} from '@/components/ui/FormSection';import {InlineAlert} from '@/components/ui/InlineAlert';
const DAYS=['saturday','sunday','monday','tuesday','wednesday','thursday','friday'] as const;const ACTS=['MEETING','TRAINING','EVENT','SALES_REVIEW_ADMIN','OTHERS'] as const;
type Cell={hospitalIds:string[];branchIds:string[];doctorIds:string[];pharmacyIds:string[];activities:string[];salesReviewDescription:string;othersDescription:string};type Plan=Record<string,{am:Cell;pm:Cell}>;type Lists={hospitals:Array<{id:string;name:string}>;branches:Array<{id:string;name:string}>;doctors:Array<{id:string;name:string;specialty?:string}>;pharmacies:Array<{id:string;name:string}>};
const cell=():Cell=>({hospitalIds:[],branchIds:[],doctorIds:[],pharmacyIds:[],activities:[],salesReviewDescription:'',othersDescription:''});const fresh=():Plan=>Object.fromEntries(DAYS.map(d=>[d,{am:cell(),pm:cell()}]));
function range(date=new Date()){const x=new Date(date),diff=(x.getDay()+1)%7;x.setDate(x.getDate()-diff);const y=new Date(x);y.setDate(x.getDate()+6);const f=(d:Date)=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;return[f(x),f(y)] as const}
export function WeeklyPlanView({reps=[],selectedRep,isManagerPersonal,initialPlan,onSuccess,onError,onPlanSaved}: {reps?:Representative[];selectedRep?:string;isManager?:boolean;isManagerPersonal?:boolean;currentUser?:{id:string;name:string;username:string}|null;initialPlan?:WeeklyPlanRecord|null;onSuccess?:(x:string)=>void;onError?:(x:string)=>void;onPlanSaved?:(plan:WeeklyPlanRecord)=>void}){
 const[startDate,setStartDate]=useState(initialPlan?.startDate||range()[0]),[endDate,setEndDate]=useState(initialPlan?.endDate||range()[1]),[repId,setRepId]=useState(initialPlan?.selectedRepId||(!isManagerPersonal?reps.find(r=>r.name===selectedRep)?.id||'':'')),[plan,setPlan]=useState<Plan>(()=>{try{return initialPlan?.structuredPlan?(typeof initialPlan.structuredPlan==='string'?JSON.parse(initialPlan.structuredPlan):initialPlan.structuredPlan):fresh()}catch{return fresh()}}),[lists,setLists]=useState<Lists>({hospitals:[],branches:[],doctors:[],pharmacies:[]}),[saving,setSaving]=useState(false),[error,setError]=useState('');
