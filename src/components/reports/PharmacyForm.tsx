...
export function PharmacyForm({
  ...
  const [pharmacies, setPharmacies] = useState<Item[]>([]);
  ...
  useEffect(() => {
    fetch('/api/lists')
      .then((r) => r.json())
      .then((a) => {
        setPharmacies(a.data?.pharmacies || []);
      })
      .catch(() => {});
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!pharmacyId || (visitType === 'Double' && !companion.trim())) {
      setError(
        l(
          'Select a pharmacy, and enter a companion for a Double visit.',
          'اختر الصيدلية وأدخل المرافق للزيارة المشتركة.'
        )
      );
      return;
    }
    setSaving(true);
    try {
      const r = await fetch('/api/reports/pharmacy', {
        method: 'POST',
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <SavedCustomerDetails
        category="pharmacy"
        customer={pharmacies.find((x) => x.id === pharmacyId)}
        visitMode={visitType}
        companion={companion}
