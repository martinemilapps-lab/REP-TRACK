  ...
  return <div className="app-shell">
    ...
    <div className="min-w-0 flex-1">
      <header className="app-header">
        <Button variant="ghost" size="sm" className="lg:hidden" aria-label={language==='ar'?'فتح التنقل':'Open navigation'} onClick={() => setOpen(true)}><Menu className="size-5"/></Button>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold truncate">{user.name}</p>
          <p className="text-xs text-[var(--ink-soft)]">{user.positionCode} · {user.username}</p>
        </div>
        <ReportingTimer />
        <Button variant="secondary" size="sm" onClick={toggleLanguage} aria-label={language==='ar'?'تغيير اللغة':'Switch language'}><Languages className="size-4"/><span>{language === 'ar' ? 'EN' : 'عربي'}</span></Button>
        <Button variant="ghost" size="sm" onClick={onLogout} aria-label={language === 'ar' ? 'تسجيل الخروج' : 'Log out'}><LogOut className="size-4"/></Button>
      </header>
