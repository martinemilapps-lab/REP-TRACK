...
export async function createDoctorVisit(
  ...
  if(modern.success){const repId=resolveWritableRepId(session);const targetDate=modern.data.visitDate||new Date().toLocaleDateString('en-CA',{timeZone:'Africa/Cairo'});assertReportSubmissionOpen(targetDate);const doctor=await db.select().from(doctors).where(and(eq(doctors.id,modern.data.doctorId),eq(doctors.repId,repId))).get();if(!doctor)throw new AppError('Doctor is not in your saved list',403);const names=await requireBusinessProducts(modern.data.products.map(x=>x.productId));const visitId=crypto.randomUUID();const latestRate=modern.data.products[0]?.prescriptionRate || null;const ops=[db.insert(doctorVisits).values({id:visitId,repId,doctorId:doctor.id,prescriptionRate:latestRate,visitDate:targetDate,cycleDays:doctor.defaultCycle||7,visitType:modern.data.visitType,companion:modern.data.visitType==='Double'?modern.data.companion:null,notes:modern.data.notes?.trim()||null}),...modern.data.products.map((item,displayOrder)=>db.insert(doctorVisitProducts).values({id:crypto.randomUUID(),doctorVisitId:visitId,productId:item.productId,productNameSnapshot:names.get(item.productId)!,prescriptionRate:item.prescriptionRate,displayOrder}))];const[first,...rest]=ops;await db.batch([first,...rest]);return{id:visitId,doctorName:doctor.name,specialty:doctor.specialty,products:modern.data.products,notes:modern.data.notes}}
