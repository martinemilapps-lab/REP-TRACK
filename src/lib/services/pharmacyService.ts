...
import { z } from 'zod';
import { PharmacyVisitSchema, PharmacyVisitV3Schema } from '@/lib/validation';
...
export async function createPharmacyVisit(
  ...
  const modern=PharmacyVisitV3Schema.safeParse(rawInput);if(modern.success){const repId=resolveWritableRepId(session);const targetDate=modern.data.visitDate||new Date().toLocaleDateString('en-CA',{timeZone:'Africa/Cairo'});assertReportSubmissionOpen(targetDate);const pharmacy=await db.select().from(pharmacies).where(and(eq(pharmacies.id,modern.data.pharmacyId),eq(pharmacies.repId,repId))).get();if(!pharmacy)throw new AppError('Pharmacy is not in your saved list',403);const productIds=modern.data.productIds||[];const names=productIds.length?await requireBusinessProducts(productIds):new Map();const [visit]=await db.insert(pharmacyVisits).values({repId,pharmacyId:pharmacy.id,lastVisitDate:targetDate,cycleDays:pharmacy.defaultCycle||7,visitType:modern.data.visitType,companion:modern.data.visitType==='Double'?modern.data.companion:null,ourProducts:productIds.length?JSON.stringify(productIds.map(productId=>({productId,name:names.get(productId)}))):null,notes:modern.data.notes?.trim()||null}).returning();return{...visit,pharmacyName:pharmacy.name}}
