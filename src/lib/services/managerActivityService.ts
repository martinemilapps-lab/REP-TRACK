...
async function hydrate(row:typeof managerActivities.$inferSelect,userName?:string,userPosition?:string):Promise<ManagerActivityRecord>{const[e,ext,nested,prods]=await Promise.all([db.select().from(managerActivityEntries).where(eq(managerActivityEntries.activityId,row.id)).all(),db.select().from(managerActivityExtendedEntries).where(eq(managerActivityExtendedEntries.activityId,row.id)).all(),db.select().from(managerActivityEntryDoctors).all(),db.select().from(managerActivityProducts).where(eq(managerActivityProducts.activityId,row.id)).all()]);let a:{activities?:string[];salesReviewDescription?:string;othersDescription?:string}={};try{a=JSON.parse(row.activitySelections||'{}')}catch{}const parse=(x:string|null)=>{try{return x?JSON.parse(x):undefined}catch{return undefined}};const visits:ManagerActivityRecord['visits']=[...e.map(v=>({id:v.id,period:v.period,entryType:v.entryType,hospitalId:v.hospitalId||undefined,doctorId:v.doctorId||undefined,name:v.nameSnapshot,specialty:v.specialtySnapshot||undefined,manualData:parse(v.manualData),generalComment:v.generalComment||undefined,displayOrder:v.displayOrder,doctors:nested.filter(d=>d.entryId===v.id).sort((x,y)=>x.displayOrder-y.displayOrder).map(d=>({id:d.id,doctorId:d.doctorId,name:d.nameSnapshot,specialty:d.specialtySnapshot||undefined,generalComment:d.generalComment||undefined}))})),...ext.map(v=>({id:v.id,period:v.period,entryType:v.entryType,pharmacyId:v.pharmacyId||undefined,branchId:v.branchId||undefined,name:v.nameSnapshot,manualData:parse(v.manualData),generalComment:v.generalComment||undefined,displayOrder:v.displayOrder,doctors:[]}))].sort((x,y)=>x.displayOrder-y.displayOrder);return{...row,userName,userPosition,selectedRepId:row.selectedRepId||undefined,...a,productsDiscussed:prods.map(p=>({id:p.productId,name:p.productNameSnapshot})),visits,submittedAt:row.submittedAt.toISOString(),updatedAt:row.updatedAt.toISOString()} as ManagerActivityRecord}
export async function getManagerActivities(session:UserSessionPayload|null,filters:{startDate?:string;endDate?:string;activityType?:string}={}){assertManagerSession(session);const f=ManagerActivityFiltersSchema.parse(filters),c=[eq(managerActivities.userId,session.id)];if(f.startDate)c.push(gte(managerActivities.activityDate,f.startDate));if(f.endDate)c.push(lte(managerActivities.activityDate,f.endDate));const rows=await db.select().from(managerActivities).where(and(...c)).orderBy(desc(managerActivities.activityDate)).all();return Promise.all(rows.map(r=>hydrate(r,session.name,session.positionCode||undefined)))}
export async function getManagerActivitiesForTeam(session:UserSessionPayload|null,f:{startDate?:string;endDate?:string;activityType?:string;subordinateUserId?:string;positionCode?:string;scopeMode?:HierarchyScopeMode}={}){assertManagerSession(session);let ids=await hierarchyService.getScopedUserIds(session,f.scopeMode);if(f.subordinateUserId){if(!ids.includes(f.subordinateUserId))throw new AppError('Employee is outside your authorized hierarchy',403);ids=[f.subordinateUserId]}if(!ids.length)return[];const c=[inArray(managerActivities.userId,ids)];if(f.startDate)c.push(gte(managerActivities.activityDate,f.startDate));if(f.endDate)c.push(lte(managerActivities.activityDate,f.endDate));const rows=await db.select({activity:managerActivities,name:users.name,position:users.positionCode}).from(managerActivities).innerJoin(users,eq(managerActivities.userId,users.id)).where(and(...c)).orderBy(desc(managerActivities.activityDate)).all();return Promise.all(rows.map(r=>hydrate(r.activity,r.name,r.position||undefined)))}
export async function getManagerActivityById(session:UserSessionPayload|null,id:string){assertManagerSession(session);const row=await db.select().from(managerActivities).where(eq(managerActivities.id,id)).get();if(!row)return null;await hierarchyService.assertUserVisible(session,row.userId);const u=await db.select({name:users.name,position:users.positionCode}).from(users).where(eq(users.id,row.userId)).get();return hydrate(row,u?.name,u?.position||undefined)}
export async function deleteManagerActivity(session:UserSessionPayload|null,id:string){assertManagerSession(session);const row=await db.select().from(managerActivities).where(eq(managerActivities.id,id)).get();if(!row)return false;assertManagerOwner(session,row.userId);await db.delete(managerActivities).where(and(eq(managerActivities.id,id),eq(managerActivities.userId,session.id)));return true}
